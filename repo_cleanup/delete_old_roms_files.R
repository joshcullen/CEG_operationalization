...
### Script to delete processed ROMS files directly from GH repo dir
...
DIRECTORIES_TO_CLEAN <- c(
  "data_acquisition/netcdfs/roms_ncdfs",
  "data_processing/ROMS/rasters"
)
...
if (DRY_RUN) {
  ...
  commit_payload <- list(
    message = paste("Automated cleanup: Batch deleted", length(paths_to_delete), "old ROMS files"),
    tree = new_tree_sha,
