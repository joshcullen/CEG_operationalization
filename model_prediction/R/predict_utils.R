...
make_png <- function(pred_rast_dir, get_date, savename, pred_img_dir, tool) {
  ...
  plot_sp <- ggplot() +
    ...
    theme_classic() +
    labs(x = NULL, y = NULL) +
    coord_sf(xlim = spat_ext[1:2], ylim = spat_ext[3:4], expand = FALSE) +
  ...
  switch(
    tool,
    "TopPredatorWatch" = ggsave(
      filename = glue("{pred_img_dir}/{savename}_{get_date}.png"),
      plot = plot_sp,
      width = 32,
      height = 22,
      units = "cm",
